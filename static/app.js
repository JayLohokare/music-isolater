document.addEventListener('DOMContentLoaded', () => {
    ...
    const API_BASE = isLocal ? 'http://localhost:7860' : 'https://jaylohokare-aura-splitter.hf.space';
    ...
    async function handleFile(file) {
        ...
        if (!validTypes.includes(file.type) && !file.name.match(/\.(mp3|wav|flac|ogg|m4a|mp4|mov|avi|mkv)$/i)) {
            showError("Invalid file type. Please upload a supported audio or video file.");
            return;
        }
    ...
    function renderResults(taskId, files) {
        ...
        files.forEach(file => {
            const nameWithoutExt = file.replace('.mp3', '').replace('.wav', '');
                ...
                <div class="track-item">
                    ...
                    <div class="track-info">
                        ...
                        <audio class="custom-audio-player" controls preload="none">
                            <source src="${API_BASE}/listen/${taskId}/${file}" type="audio/mpeg">
                            Your browser does not support the audio element.
                        </audio>
                    </div>
                    <a href="${API_BASE}/download/${taskId}/${file}" class="download-btn" title="Download Setup">
